...
it('lets 2 users exchange stars', async() => {
    ...
    // if sender is accounts[0], accounts[1] need approve for accounts[0] to transfer tokenId2.
    ...
    // Owner of tokenId1 should be user2.
    assert.equal(await instance.ownerOf.call(tokenId1), accounts[1]);

    // Owner of tokenId2 shoule be user1.
...
it('lets a user transfer a star', async() => {
    ...
    // owner of the tokenId should be accounts[1]
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    // 1. create a Star with different tokenId
    let tokenId = 10;
    await instance.createStar('Wayne Rooney', tokenId, {from: accounts[0]});

    // 2. Call your method lookUptokenIdToStarInfo
    const starName = await instance.lookUptokenIdToStarInfo.call(tokenId, {from: accounts[0]});
    
    // 3. Verify if you Star name is the same
    assert.equal(starName, 'Wayne Rooney');
});